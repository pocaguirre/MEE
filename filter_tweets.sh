...
INPUT_DIR="/export/c12/mdredze/twitter/public"
OUTPUT_DIR="/export/b03/caguirr/emoji-eval/twitter_data/raw"
mkdir -p "${OUTPUT_DIR}"
...
do
  ...
  do
    # Add to input files
    INPUT_FILES+=( "${INPUT_DIR}/${y}/${m}"/*.gz )

    # Create corresponding directory if it does not exist
    mkdir -p "${OUTPUT_DIR}/${y}/${m}"
